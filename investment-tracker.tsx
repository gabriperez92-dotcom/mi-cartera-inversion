...
export default function App() {
  ...
  const save = useCallback(async (f, e) => {
    try {
      await window.storage?.set(STORAGE_KEY, JSON.stringify({ funds: f, entries: e }));
      setSaved(true);
      setTimeout(() => setSaved(false), 1500);
    } catch {}
  }, []);

  const persist = (f, e) => { setFunds(f); setEntries(e); save(f, e); };
    ...
    <div style={{ fontFamily: "system-ui, sans-serif", background: "#f0f4f8", minHeight: "100vh", padding: "16px" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
          {[["resumen", "📋 Resumen"], ["aportaciones", "➕ Aportaciones"], ["fondos", "⚙️ Gestionar Fondos"]].map(([k, l]) => (
